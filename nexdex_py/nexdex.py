...
class NexDex:
    """NexDex class to interact with NexDex REST API.
    ...
    Examples:
        ...
        >>> # With custom timeout
        >>> NexDex = NexDex(env=Environment.TESTNET, ws_timeout=30)
        >>> # With simulator-friendly injection (high-frequency, no sleeps)
        >>> NexDex = NexDex(env=Environment.TESTNET, auto_start_ws_reader=False,
    ...
    def __init__(
        ...
        if env is None:
            return raise_value_error("NexDex: Invalid environment")
        self.env = env
        ...
        self.api_client = NexDexApiClient(
            env=env,
            logger=logger,
        ...
        self.ws_client = NexDexWebsocketClient(
            env=env,
            logger=logger,
    ...
    def init_account(
        ...
        """Initialize NexDex account with l1 or l2 private keys.
        ...
        if self.account is not None:
            return raise_value_error("NexDex: Account already initialized")
        self.account = NexDexAccount(
            config=self.config,
            l1_address=l1_address,
            l1_private_key=l1_private_key,
            l2_private_key=l2_private_key,
    ...
    async def close(self):
        ...
        This method should be called when done using the NexDex instance
    ...
    def __del__(self):
        """Cleanup when NexDex instance is destroyed.
