...
class NexDexApiClient(BlockTradesMixin, HttpClient):
    """Class to interact with NexDex REST API.
    ...
    def __init__(
        ...
        if api_base_url is not None:
            ...
            self.api_url = f"https://api.{self.env}.NexDex.trade/v1"
    ...
    def init_account(self, account: NexDexAccount):
    ...
    def fetch_order(self, order_id: str) -> dict:
        ...
        Args:
            order_id: order's id as assigned by NexDex.
        """
    ...
    def fetch_trades(self, params: dict) -> dict:
        """Fetch NexDex exchange trades for specific market.
    ...
    def submit_orders_batch(self, orders: list[Order], signer: Signer | None = None) -> dict:
        """Send batch of orders to NexDex.
            Private endpoint requires authorization.
    ...
    def modify_order(self, order_id: str, order: Order, signer: Signer | None = None) -> dict:
        """Modify an open order previously sent to NexDex from this account.
            Private endpoint requires authorization.
    ...
    def cancel_order(self, order_id: str) -> None:
        """Cancel open order previously sent to NexDex from this account.
            Private endpoint requires authorization.
    ...
    def cancel_orders_batch(
        ...
        Args:
            order_ids: List of order IDs assigned by NexDex
    ...
    def fetch_system_config(self) -> SystemConfig:
        """Fetch NexDex system config.

        Examples:
            >>> NexDex.api_client.fetch_system_config()
    ...
    def fetch_system_state(self) -> dict:
        """Fetch NexDex system status.

        Examples:
            >>> NexDex.api_client.fetch_system_state()
            >>> { "status": "ok" }
        """
        return self._get(path="system/state")

    def fetch_system_time(self) -> dict:
        """Fetch NexDex system time.

        Examples:
            >>> NexDex.api_client.fetch_system_time()
            >>> { "server_time": "1710956478221" }

        Returns:
            server_time: NexDex Server time
        """
