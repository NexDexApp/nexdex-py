...
from nexdex_py.account.subkey_account import SubkeyAccount
...
class NexDexSubkey:
    ...
    This class extends NexDex functionality but uses SubkeyAccount for L2-only
    ...
    def __init__(
        ...
        if env is None:
            return raise_value_error("NexDexSubkey: Invalid environment")

        # Validate required parameters
        if not l2_private_key:
            raise_value_error("NexDexSubkey: L2 private key is required")
        if not l2_address:
            raise_value_error("NexDexSubkey: L2 address is required")

        self.env = env
        self.logger: logging.Logger = logger or logging.getLogger(__name__)

        # Load api client and system config
        self.api_client = NexDexApiClient(env=env, logger=logger)
        self.ws_client = NexDexWebsocketClient(env=env, logger=logger, ws_timeout=ws_timeout)
    ...
    async def init_account(self):
        ...
        This method is provided for compatibility with the NexDex interface,
