"""Tests for simulator-friendly injection points in nexdex_py."""
import asyncio
import json
...
from nexdex_py.api.ws_client import NexDexWebsocketChannel, NexDexWebsocketClient
from nexdex_py.environment import TESTNET
from nexdex_py.NexDex import NexDex
...
class TestHttpClientInjection:
    ...
    def test_mock_transport_integration(self):
        ...
        from nexdex_py.api.http_client import HttpMethod
...
class TestApiClientInjection:
    ...
    def test_api_client_default_initialization(self):
        ...
        with patch.object(NexDexApiClient, "fetch_system_config", return_value=MagicMock()):
            client = NexDexApiClient(env=TESTNET)
    ...
    def test_api_client_http_injection(self):
        ...
        with patch.object(NexDexApiClient, "fetch_system_config", return_value=MagicMock()):
            client = NexDexApiClient(env=TESTNET, http_client=custom_http_client)
    ...
    def test_api_client_base_url_override(self):
        ...
        with patch.object(NexDexApiClient, "fetch_system_config", return_value=MagicMock()):
            client = NexDexApiClient(env=TESTNET, api_base_url=custom_url)
    ...
    def test_api_client_both_injections(self):
        ...
        with patch.object(NexDexApiClient, "fetch_system_config", return_value=MagicMock()):
            client = NexDexApiClient(env=TESTNET, http_client=custom_http_client, api_base_url=custom_url)
            assert client.api_url == custom_url
            assert client.client is not None
...
class TestWebSocketClientInjection:
    ...
    def test_ws_client_default_initialization(self):
        ...
        client = NexDexWebsocketClient(env=TESTNET)
        assert client.env == TESTNET
        assert "testnet" in client.api_url
    ...
    def test_ws_client_auto_start_reader_disabled(self):
        ...
        client = NexDexWebsocketClient(env=TESTNET, auto_start_reader=False)
    ...
    def test_ws_client_url_override(self):
        ...
        client = NexDexWebsocketClient(env=TESTNET, ws_url_override=custom_url)
        assert client.api_url == custom_url
    ...
    async def test_ws_client_custom_connector(self):
        ...
        client = NexDexWebsocketClient(env=TESTNET, auto_start_reader=False, connector=mock_connector)
    ...
    async def test_ws_client_manual_pumping(self):
        ...
        await client.subscribe(NexDexWebsocketChannel.BBO, callback=test_callback, params={"market": "BTC-USD-PERP"})
    ...
    async def test_ws_client_message_injection(self):
        ...
        assert channel == NexDexWebsocketChannel.BBO
        assert message["data"]["bid"] == "50000"


class TestNexDexFacadeInjection:
    """Test NexDex facade injection functionality."""

    @patch.object(NexDexApiClient, "fetch_system_config", return_value=MagicMock())
    def test_NexDex_default_initialization(self, mock_config):
        """Test default NexDex initialization."""
        NexDex = NexDex(env=TESTNET)
        assert NexDex.env == TESTNET
        assert NexDex.api_client is not None
        assert NexDex.ws_client is not None

    @patch.object(NexDexApiClient, "fetch_system_config", return_value=MagicMock())
    def test_NexDex_http_injection(self, mock_config):
        """Test NexDex with HTTP client injection."""
        custom_http_client = HttpClient(http_client=httpx.Client())

        NexDex = NexDex(env=TESTNET, http_client=custom_http_client)

        assert NexDex.api_client is not None
        # Verify the injection was passed through
        assert NexDex.api_client.client is not None

    @patch.object(NexDexApiClient, "fetch_system_config", return_value=MagicMock())
    def test_NexDex_ws_injection(self, mock_config):
        """Test NexDex with WebSocket injection."""
        ...
        assert NexDex.ws_client.auto_start_reader is False
        assert NexDex.ws_client.connector is mock_connector
        assert NexDex.ws_client.api_url == "wss://custom.example.com/v1"

    @patch.object(NexDexApiClient, "fetch_system_config", return_value=MagicMock())
    def test_NexDex_full_injection(self, mock_config):
        """Test NexDex with all injection options."""
        custom_http_client = HttpClient(http_client=httpx.Client())
        ...
        assert NexDex.api_client.api_url == custom_api_url
        assert NexDex.ws_client.api_url == custom_ws_url
        assert NexDex.ws_client.auto_start_reader is False
        assert NexDex.ws_client.connector is mock_connector
...
class TestSimulatorUseCases:
    ...
    def test_rest_simulator_integration(self):
        ...
        assert isinstance(api_client, NexDexApiClient)
    ...
    async def test_ws_simulator_integration(self):
        ...
        await ws_client.subscribe(NexDexWebsocketChannel.BBO, callback=data_handler, params={"market": "BTC-USD-PERP"})
    ...
    async def test_combined_simulator_integration(self):
        ...
        # Create fully injected NexDex instance
        NexDex = NexDex(
            env=TESTNET,
        ...
        assert NexDex.api_client.api_url == "https://rest-sim.example.com/v1"
        assert NexDex.ws_client.api_url == "wss://ws-sim.example.com/v1"
        assert NexDex.ws_client.auto_start_reader is False
        ...
        await NexDex.ws_client.subscribe(
            NexDexWebsocketChannel.BBO, callback=handler, params={"market": "BTC-USD-PERP"}
        )

        await NexDex.ws_client.pump_once()
        assert len(received_messages) == 1
