"""Tests for NexDexAccount httpx integration."""
...
class TestAccountHttpxIntegration:
    """Test suite for NexDexAccount's httpx integration via monkey patching."""
    ...
    async def test_monkey_patched_make_request_success(self):
        ...
        with patch.object(
            ...
            assert "NexDex-STARKNET-ACCOUNT" in call_args[1]["headers"]
            assert "NexDex-STARKNET-SIGNATURE" in call_args[1]["headers"]
    ...
    async def test_monkey_patched_request_with_headers(self):
        ...
        # Check that all required NexDex headers are present
        required_headers = [
            "NexDex-STARKNET-ACCOUNT",
            "NexDex-STARKNET-SIGNATURE",
            "NexDex-STARKNET-SIGNATURE-TIMESTAMP",
            "NexDex-STARKNET-SIGNATURE-VERSION",
        ]
        ...
        assert headers["NexDex-STARKNET-ACCOUNT"].startswith("0x")

        # Verify signature format (should be JSON array string)
        signature = headers["NexDex-STARKNET-SIGNATURE"]
        assert signature.startswith("[") and signature.endswith("]")

        # Verify timestamp is numeric string
        timestamp = headers["NexDex-STARKNET-SIGNATURE-TIMESTAMP"]
    ...
    def test_fullnode_request_headers_generation(self):
        ...
        assert "NexDex-STARKNET-ACCOUNT" in headers
        assert "NexDex-STARKNET-SIGNATURE" in headers
        assert "NexDex-STARKNET-SIGNATURE-TIMESTAMP" in headers
        assert "NexDex-STARKNET-SIGNATURE-VERSION" in headers

        # Verify account address format
        assert headers["NexDex-STARKNET-ACCOUNT"] == hex(account.starknet.address)

        # Verify signature version
        assert headers["NexDex-STARKNET-SIGNATURE-VERSION"] == "1.0.0"
