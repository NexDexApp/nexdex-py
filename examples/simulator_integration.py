...
try:
    from nexdex_py.api.ws_models import (
        JSONRPCRequest,
        create_data_message,
...
async def demo_full_integration():
    ...
    # Full NexDex instance with all injections
    NexDex = NexDex(
        env=Environment.TESTNET,
        # REST injection
    ...
    await NexDex.ws_client.subscribe_by_name("bbo.BTC-USD-PERP", update_handler)

    # Process all messages
    while len(updates) < 5:
        result = await NexDex.ws_client.pump_once()
        if not result:
            break

    print(f"✅ WebSocket: {len(updates)} BBO updates received")
    await NexDex.ws_client._close_connection()


async def main():
    """Run all simulator integration examples."""
    print("🚀 NexDex Simulator Integration")
    print("=" * 50)
