#!/usr/bin/env python3
"""
Example script demonstrating L2-only WebSocket usage with NexDexSubkey.
...
# Add the parent directory to the path so we can import nexdex_py
...
async def main():
    ...
    print("🚀 Starting NexDex L2-only WebSocket example...")
    print(f"L2 Address: {l2_address}")
    print(f"L2 Private Key: {l2_private_key[:10]}...")

    # Initialize NexDexSubkey with L2-only credentials
    NexDex = NexDexSubkey(
        env=TESTNET,
    ...
    try:
        ...
        await NexDex.ws_client.subscribe(channel=NexDexWebsocketChannel.MARKETS_SUMMARY, callback=on_markets_summary)
        ...
        await NexDex.ws_client.subscribe(
            channel=NexDexWebsocketChannel.ORDERS, callback=on_orders, params={"market": "ALL"}
        )
        print("✅ Subscribed to orders")

        await NexDex.ws_client.subscribe(
            channel=NexDexWebsocketChannel.FILLS, callback=on_fills, params={"market": "ETH-USD-PERP"}
        )
