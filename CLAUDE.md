...
# Type checking
uv run mypy --check-untyped-defs NexDex_py
```
...
- **NexDex_py/**: Main package directory
...
1. **NexDex Client** (`NexDex_py/NexDex.py`): Main entry point
...
2. **Account Management** (`NexDex_py/account/`):
...
4. **Message Signing** (`NexDex_py/message/`):
...
- Uses `NexDex_py.environment.Environment` enum: `PROD` or `TESTNET`
