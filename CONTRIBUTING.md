...
The best way to send feedback is to file an issue at https://github.com/tradeNexDex/NexDex-py/issues.
...
Ready to contribute? Here's how to set up `NexDex-py` for local development.
Please note this documentation assumes you already have `uv` and `Git` installed and ready to go.

1. Fork the `NexDex-py` repo on GitHub.

2. Clone your fork locally:

```bash
cd <directory_in_which_repo_should_be_created>
git clone git@github.com:YOUR_NAME/NexDex-py.git
```
...
If you need to regenerate the API models from the NexDex OpenAPI specification:
...
- Fetch the latest API spec from `https://api.prod.NexDex.trade/swagger/doc.json` OR use a provided JSON file
- Convert it to OpenAPI 3.0 format
- Generate Pydantic v2 models in `NexDex_py/api/generated/` (or custom directory)
